    ...
    internal partial class MainForm : BorderedMetroForm
    {
        ...
        private void KeyboardHookKeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyCode == Keys.L && NativeMethods.GetForegroundWindow() == Handle)
            {
                ...
                //// Pattern wowBuildPattern = Pattern.FromTextstyle("WoWBuild", "8B 45 14 85 C0 74 08 8B 0D ?? ?? ?? ?? 89 08", new AddModifier(2), new LeaModifier());
                //Log.Print("GlueState: " + glueStatePattern.Find(WowProcess.GetAllWowProcesses()[0].Memory).ToInt32());
                //Log.Print("GameState: " + gameStatePattern.Find(WowProcess.GetAllWowProcesses()[0].Memory).ToInt32());
                //Log.Print("FocusedWidget: " + focusedWidgetPattern.Find(WowProcess.GetAllWowProcesses()[0].Memory).ToInt32());
                //// Log.Print("WoWBuild: " + wowBuildPattern.Find(WowProcess.GetAllWowProcesses()[0].Memory).ToInt32());
