...
namespace AxTools.Helpers.MemoryManagement
{
    ...
    internal sealed unsafe class MemoryManager : IDisposable
    {
        ...
        public bool Write<T>(IntPtr address, T value)
        {
            ...
            VirtualProtectEx(ProcessHandle, address, (IntPtr)MarshalCache<T>.Size, WinAPI.Win32Consts.PAGE_READWRITE, out uint oldProtect);
        ...
        {
            if (VirtualProtectEx(ProcessHandle, address, (IntPtr)bytes.Length, WinAPI.Win32Consts.PAGE_READWRITE, out uint oldProtect))
            {
