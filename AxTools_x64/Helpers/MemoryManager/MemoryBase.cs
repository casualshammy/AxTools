...
namespace AxTools.Helpers.MemoryManager
{
    public abstract class MemoryBase : IDisposable
    {
        ...
        public GreyMagic.SafeMemoryHandle ProcessHandle;
        ...
        /// <param name="isRelative">if set to <c>true</c> [is relative].</param>
        /// <returns></returns>
        public virtual T[] ReadStructArray<T>(IntPtr address, int elements, bool isRelative = false) where T : struct
        {
            if (isRelative)
        ...
        public virtual void MemCopy<T>(IntPtr source, IntPtr dest) where T : struct
        {
